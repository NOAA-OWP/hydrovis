DROP TABLE IF EXISTS ingest.stage_based_catfim_minor_intervals;
WITH main AS (
	SELECT
		trace_feature_id as feature_id,
		rf.nws_station_id,
		generate_series(
			(adj_minor_stage_m + 0.3048)::numeric,
			COALESCE(adj_moderate_stage_m, adj_major_stage_m, adj_minor_stage_m + 1.524)::numeric,
			0.3048
		) as stage_m
	FROM cache.rfc_categorical_stages AS rf
	WHERE adj_minor_stage_m IS NOT NULL
), groupings AS (
...
INTO ingest.stage_based_catfim_minor_intervals
...
DROP TABLE IF EXISTS ingest.stage_based_catfim_minor_intervals_job_num;
SELECT *
INTO ingest.stage_based_catfim_minor_intervals_job_num
FROM ingest.stage_based_catfim_minor_intervals
WHERE interval_ft > lower_interval AND interval_ft <= upper_interval;

----------------------------------------------------------------------------------------------------------
SELECT UpdateGeometrySRID('ingest', 'stage_based_catfim_minor_intervals', 'geom', 3857);
SELECT UpdateGeometrySRID('ingest', 'stage_based_catfim_minor_intervals_job_num', 'geom', 3857);