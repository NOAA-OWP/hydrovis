DROP TABLE IF EXISTS ingest.flow_based_catfim_major;
SELECT
    crosswalk.hand_id,
    crosswalk.hydro_id,
    crosswalk.feature_id,
    ROUND(CAST(rf.major_flow_cms * 35.315 as numeric), 2) AS forecast_discharge_cfs,
...
INTO ingest.flow_based_catfim_major
...
WHERE major_flow_cms IS NOT NULL AND
	crosswalk.huc8 IS NOT NULL AND 
    crosswalk.lake_id = -999 AND
    ((rf.major_flow_cms <= gc.discharge_cms AND rf.major_flow_cms > gc.previous_discharge_cms)
	 OR rf.major_flow_cms > mgc.max_rc_discharge_cms);

SELECT UpdateGeometrySRID('ingest', 'flow_based_catfim_major', 'geom', 3857);