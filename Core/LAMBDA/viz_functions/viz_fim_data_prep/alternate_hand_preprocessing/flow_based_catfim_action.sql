...
SELECT
    ...
    ROUND(CAST(rf.action_flow_cms * 35.315 as numeric), 2) AS forecast_discharge_cfs,
...
WHERE action_flow_cms IS NOT NULL AND
	crosswalk.huc8 IS NOT NULL AND 
    crosswalk.lake_id = -999 AND
    ((rf.action_flow_cms <= gc.discharge_cms AND rf.action_flow_cms > gc.previous_discharge_cms)
	 OR rf.action_flow_cms > mgc.max_rc_discharge_cms);

SELECT UpdateGeometrySRID('ingest', 'flow_based_catfim_action', 'geom', 3857);