DROP TABLE IF EXISTS ingest.stage_based_catfim_minor;
SELECT
    crosswalk.hand_id,
    crosswalk.hydro_id,
    crosswalk.feature_id,
    ROUND(CAST(rf.adj_minor_stage_m * 35.315 as numeric), 2) AS forecast_stage_ft,
...
INTO ingest.stage_based_catfim_minor
...
WHERE adj_minor_stage_m IS NOT NULL AND
    ...
    ((rf.adj_minor_stage_m <= gc.stage_m AND rf.adj_minor_stage_m > gc.previous_stage_m)
	 OR rf.adj_minor_stage_m > mgc.max_rc_stage_m);

SELECT UpdateGeometrySRID('ingest', 'stage_based_catfim_minor', 'geom', 3857);