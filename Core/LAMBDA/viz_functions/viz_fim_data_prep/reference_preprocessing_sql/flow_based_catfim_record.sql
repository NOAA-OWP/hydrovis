DROP TABLE IF EXISTS ingest.flow_based_catfim_record;
SELECT
    crosswalk.hand_id,
    crosswalk.hydro_id,
    crosswalk.feature_id,
    ROUND(CAST(rf.record_flow_cms * 35.315 as numeric), 2) AS forecast_discharge_cfs,
...
INTO ingest.flow_based_catfim_record
...
WHERE record_flow_cms IS NOT NULL AND
    ...
    ((rf.record_flow_cms <= gc.discharge_cms AND rf.record_flow_cms > gc.previous_discharge_cms)
	 OR rf.record_flow_cms > mgc.max_rc_discharge_cms);

SELECT UpdateGeometrySRID('ingest', 'flow_based_catfim_record', 'geom', 3857);