...
CREATE INDEX IF NOT EXISTS rfc_5day_max_downstream_inundation_geom_idx ON publish.rfc_5day_max_downstream_inundation USING GIST (geom);

--------------- Building Footprints ---------------
DROP TABLE IF EXISTS publish.rfc_5day_max_downstream_inundation_building_footprints;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
INTO publish.rfc_5day_max_downstream_inundation_building_footprints
FROM external.building_footprints_fema as buildings
JOIN publish.rfc_5day_max_downstream_inundation fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.rfc_5day_max_downstream_inundation_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.rfc_5day_max_downstream_inundation_counties
...
-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.rfc_5day_max_downstream_inundation_hucs;
SELECT
	hucs.huc8,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.rfc_5day_max_downstream_inundation_hucs
