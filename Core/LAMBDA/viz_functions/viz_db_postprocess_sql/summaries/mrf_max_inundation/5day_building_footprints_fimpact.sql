...
INTO publish.mrf_max_inundation_5day_building_footprints
FROM external.building_footprints_fema as buildings
JOIN publish.mrf_max_inundation_5day fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.mrf_max_inundation_5day_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.mrf_max_inundation_5day_counties
...
-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.mrf_max_inundation_5day_hucs;
SELECT
	hucs.huc8,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.mrf_max_inundation_5day_hucs
