--------------- Building Footprints ---------------
DROP TABLE IF EXISTS publish.srf_18hr_max_inundation_extent_building_footprints;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
INTO publish.srf_18hr_max_inundation_extent_building_footprints
FROM external.building_footprints_fema as buildings
JOIN publish.srf_18hr_max_inundation_extent fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.srf_18hr_max_inundation_extent_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.srf_18hr_max_inundation_extent_counties
FROM derived.counties AS counties
JOIN derived.channels_county_crosswalk AS crosswalk ON counties.geoid = crosswalk.geoid
JOIN publish.srf_18hr_max_inundation_extent AS fim on crosswalk.feature_id = fim.feature_id
JOIN publish.srf_18hr_max_inundation_extent_building_footprints AS buildings ON crosswalk.feature_id = buildings.feature_id
GROUP BY counties.geoid, counties.name, counties.geom, buildings.prop_st;

-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.srf_18hr_max_inundation_extent_hucs;
SELECT
	hucs.huc10,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.srf_18hr_max_inundation_extent_hucs
