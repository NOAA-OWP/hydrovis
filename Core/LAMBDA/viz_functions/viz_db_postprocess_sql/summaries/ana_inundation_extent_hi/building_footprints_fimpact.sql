--------------- Building Footprints ---------------
DROP TABLE IF EXISTS publish.ana_inundation_extent_building_footprints_hi;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
INTO publish.ana_inundation_extent_building_footprints_hi
FROM external.building_footprints_fema as buildings
JOIN publish.ana_inundation_extent_hi fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.ana_inundation_extent_counties_hi;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.ana_inundation_extent_counties_hi
FROM derived.counties AS counties
JOIN derived.channels_county_crosswalk AS crosswalk ON counties.geoid = crosswalk.geoid
JOIN publish.ana_inundation_extent_hi AS fim on crosswalk.feature_id = fim.feature_id
JOIN publish.ana_inundation_extent_building_footprints_hi AS buildings ON crosswalk.feature_id = buildings.feature_id
