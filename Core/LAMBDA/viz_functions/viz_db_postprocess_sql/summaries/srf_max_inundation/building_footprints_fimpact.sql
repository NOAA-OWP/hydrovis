...
CREATE INDEX IF NOT EXISTS srf_max_inundation_geom_idx ON publish.srf_max_inundation USING GIST (geom);

--------------- Building Footprints ---------------
DROP TABLE IF EXISTS publish.srf_max_inundation_building_footprints;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
INTO publish.srf_max_inundation_building_footprints
FROM external.building_footprints_fema as buildings
JOIN publish.srf_max_inundation fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.srf_max_inundation_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.srf_max_inundation_counties
FROM derived.counties AS counties
JOIN derived.channels_county_crosswalk AS crosswalk ON counties.geoid = crosswalk.geoid
JOIN publish.srf_max_inundation AS fim on crosswalk.feature_id = fim.feature_id
JOIN publish.srf_max_inundation_building_footprints AS buildings ON crosswalk.feature_id = buildings.feature_id
GROUP BY counties.geoid, counties.name, counties.geom, buildings.prop_st;

-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.srf_max_inundation_hucs;
SELECT
	hucs.huc10,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.srf_max_inundation_hucs
