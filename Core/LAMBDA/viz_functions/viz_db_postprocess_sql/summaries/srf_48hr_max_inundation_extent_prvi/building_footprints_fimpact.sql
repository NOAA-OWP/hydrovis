--------------- Building Footprints ---------------
DROP TABLE IF EXISTS publish.srf_48hr_max_inundation_extent_building_footprints_prvi;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
INTO publish.srf_48hr_max_inundation_extent_building_footprints_prvi
FROM external.building_footprints_fema as buildings
JOIN publish.srf_48hr_max_inundation_extent_prvi fim ON ST_INTERSECTS(fim.geom, buildings.geom);

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.srf_48hr_max_inundation_extent_counties_prvi;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.srf_48hr_max_inundation_extent_counties_prvi
FROM derived.counties AS counties
JOIN derived.channels_county_crosswalk AS crosswalk ON counties.geoid = crosswalk.geoid
JOIN publish.srf_48hr_max_inundation_extent_prvi AS fim on crosswalk.feature_id = fim.feature_id
JOIN publish.srf_48hr_max_inundation_extent_building_footprints_prvi AS buildings ON crosswalk.feature_id = buildings.feature_id
