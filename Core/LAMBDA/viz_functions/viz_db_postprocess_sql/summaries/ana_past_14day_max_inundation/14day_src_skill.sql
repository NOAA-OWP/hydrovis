...
SELECT
    ...
    ana.discharge_cfs AS max_flow_14day_cfs,
    ...
    MIN(urc.elevation_ft) + ((ana.discharge_cfs - MIN(urc.discharge_cfs)) * ((MAX(urc.next_elevation_ft) - MIN(urc.elevation_ft)) / (MAX(urc.next_discharge_cfs) - MIN(urc.discharge_cfs)))) as diff_ft,
...
JOIN derived.recurrence_flows_conus thresholds ON ana.feature_id = thresholds.feature_id AND ana.discharge_cfs >= thresholds.high_water_threshold
JOIN derived.hydrotable_staggered AS ht ON ht.feature_id = ana.feature_id AND ana.discharge_cfs >= ht.discharge_cfs AND ana.discharge_cfs <= ht.next_discharge_cfs
JOIN derived.usgs_rating_curves_staggered AS urc ON urc.location_id::text = ht.location_id AND ana.discharge_cfs >= urc.discharge_cfs AND ana.discharge_cfs <= urc.next_discharge_cfs
JOIN external.usgs_gage AS gage ON LPAD(gage.usgs_gage_id::text, 8, '0') = LPAD(ht.location_id::text, 8, '0')
GROUP BY urc.location_id, ht.feature_id, ana.discharge_cfs;
