...
JOIN derived.hydrotable_ms_staggered AS ht ON ht.feature_id = ana.feature_id AND ana.max_flow_7day_cfs >= ht.discharge_cfs AND ana.max_flow_7day_cfs <= ht.next_discharge_cfs
JOIN derived.usgs_rating_curves_staggered AS urc ON urc.location_id = ht.location_id AND ana.max_flow_7day_cfs >= urc.discharge_cfs AND ana.max_flow_7day_cfs <= urc.next_discharge_cfs
