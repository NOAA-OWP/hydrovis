DROP TABLE IF EXISTS publish.mrf_gfs_peak_flow_arrival_time_alaska;

WITH arrival_time AS(
    SELECT 
           forecasts.feature_id, 
           max(forecasts.forecast_hour)+1 AS t_normal
    FROM ingest.nwm_channel_rt_mrf_gfs_alaska AS forecasts
...
INTO publish.mrf_gfs_peak_flow_arrival_time_alaska
FROM ingest.nwm_channel_rt_mrf_gfs_alaska AS forecasts
