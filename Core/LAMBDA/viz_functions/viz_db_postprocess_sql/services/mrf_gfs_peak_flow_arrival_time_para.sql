DROP TABLE IF EXISTS publish.mrf_gfs_peak_flow_arrival_time_para;
...
INTO publish.mrf_gfs_peak_flow_arrival_time_para
FROM ingest.nwm_channel_rt_mrf_gfs_para AS forecasts

-- Join in max flows on max streamflow to only get peak flows
JOIN cache.mrf_gfs_max_flows_para AS max_flows
