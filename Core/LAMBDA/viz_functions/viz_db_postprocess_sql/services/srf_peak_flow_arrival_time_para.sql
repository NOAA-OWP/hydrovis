DROP TABLE IF EXISTS publish.srf_peak_flow_arrival_time_para;
...
INTO publish.srf_peak_flow_arrival_time_para
FROM ingest.nwm_channel_rt_srf_para AS forecasts 

-- Join in max flows on max streamflow to only get peak flows
JOIN cache.max_flows_srf_para AS max_flows
...
GROUP BY forecasts.feature_id, forecasts.reference_time, forecasts.nwm_vers, channels.name, channels.strm_order, channels.huc6, channels.nwm_vers, rf.high_water_threshold, arrival_time.t_normal, max_flows.maxflow_18hour_cms, channels.geom