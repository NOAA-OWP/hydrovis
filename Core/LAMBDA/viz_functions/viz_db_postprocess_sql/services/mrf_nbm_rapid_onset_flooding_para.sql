DROP TABLE IF EXISTS publish.mrf_nbm_rapid_onset_flooding_para;
			...
			WITH forecasts_full AS
				...
				WITH series AS -- Calculate a full 240 hour series for every feature_id, so that unadjacent hours aren't compared
					(SELECT channels.feature_id, generate_series(3,240,3) AS forecast_hour
					 FROM derived.channels_conus channels JOIN cache.mrf_nbm_max_flows_para as mf on channels.feature_id = mf.feature_id
				...
				FROM series
				LEFT OUTER JOIN ingest.nwm_channel_rt_mrf_nbm_para AS forecasts ON series.feature_id = forecasts.feature_id AND series.forecast_hour = forecasts.forecast_hour -- Left outer join to the forecasts table (so that all hours are always present)
...
SELECT channels.feature_id,
	...
	geom
INTO publish.mrf_nbm_rapid_onset_flooding_para
