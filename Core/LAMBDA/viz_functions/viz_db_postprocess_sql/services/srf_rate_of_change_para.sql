DROP TABLE IF EXISTS publish.srf_rate_of_change_para;

WITH roi AS (
    SELECT max_srf.feature_id, thresholds.high_water_threshold
    FROM cache.max_flows_srf_para AS max_srf
...
INTO publish.srf_rate_of_change_para
FROM ingest.nwm_channel_rt_srf_para as srf
JOIN roi ON roi.feature_id = srf.feature_id
JOIN cache.max_flows_ana_past_hour_para AS ana ON ana.feature_id = srf.feature_id
