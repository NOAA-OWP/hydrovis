DROP TABLE IF EXISTS publish.srf_rapid_onset_flooding_para;
...
WITH rapid_onset AS (
		...
		WITH pct_change_by_hour AS
			...
			WITH forecasts_full AS
				...
				WITH series AS -- Calculate a full 18 hour series for every feature_id, so that unadjacent hours aren't compared
					(SELECT channels.feature_id, generate_series(1,18,1) AS forecast_hour
					 FROM derived.channels_conus channels JOIN cache.max_flows_srf_para as mf on channels.feature_id = mf.feature_id
				...
				FROM series
				LEFT OUTER JOIN ingest.nwm_channel_rt_srf_para AS forecasts ON series.feature_id = forecasts.feature_id AND series.forecast_hour = forecasts.forecast_hour -- Left outer join to the forecasts table (so that all hours are always present)
	...
	FROM ingest.nwm_channel_rt_srf_para AS forecasts
...
SELECT channels.feature_id,
	...
	geom
INTO publish.srf_rapid_onset_flooding_para
