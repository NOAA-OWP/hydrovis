...
LEFT OUTER JOIN handfim_cache.{hand_version_db}__hydrotable_cached_max AS hcm ON fim.hand_id = hcm.hand_id
...
FROM {db_fim_table} AS fim
LEFT OUTER JOIN handfim_cache.{hand_version_db}__hydrotable_cached AS hc ON fim.hand_id = hc.hand_id AND fim.rc_stage_ft = hc.rc_stage_ft
...
INSERT INTO handfim_cache.{hand_version_db}__hydrotable_cached_geo (hand_id, rc_stage_ft, geom)
SELECT
    fim_geo.hand_id,
    fim_geo.rc_stage_ft,
    fim_geo.geom
...
LEFT OUTER JOIN handfim_cache.{hand_version_db}__hydrotable_cached_geo AS hcg ON fim_geo.hand_id = hcg.hand_id AND fim_geo.rc_stage_ft = hcg.rc_stage_ft
...
INSERT INTO handfim_cache.{hand_version_db}__hydrotable_cached_zero_stage (hand_id, rc_discharge_cms, note)
SELECT
    fim.hand_id,
    fim.rc_discharge_cms,
    fim.note
FROM {db_fim_table}_zero_stage AS fim
LEFT OUTER JOIN handfim_cache.{hand_version_db}__hydrotable_cached_zero_stage AS hczs ON fim.hand_id = hczs.hand_id
