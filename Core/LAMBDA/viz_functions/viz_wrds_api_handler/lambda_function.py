...
def lambda_handler(event, context):
    ...
    upload_df_to_s3(df_meta, PROCESSED_OUTPUT_BUCKET, meta_key)
    ...
    upload_df_to_s3(df_forecasts, PROCESSED_OUTPUT_BUCKET, forecast_key)

    es_logger.info(f"Triggering DB ingest for ahps forecast and metadata for {reference_time}")
    trigger_db_ingest("ahps", reference_time, PROCESSED_OUTPUT_BUCKET, forecast_key)
...
def cleanup_cache(reference_date, buffer_days=.25):
    ...
    for fifteen_min_increments in range(1, buffer_increments+1):
        ...
        if check_s3_file_existence(PROCESSED_OUTPUT_BUCKET, metadata_csv_key):
            s3_resource.Object(PROCESSED_OUTPUT_BUCKET, metadata_csv_key).delete()
            print(f"Deleted file {metadata_csv_key} from {PROCESSED_OUTPUT_BUCKET}")

        if check_s3_file_existence(PROCESSED_OUTPUT_BUCKET, forecast_csv_key):
            s3_resource.Object(PROCESSED_OUTPUT_BUCKET, forecast_csv_key).delete()
            print(f"Deleted file {forecast_csv_key} from {PROCESSED_OUTPUT_BUCKET}")
