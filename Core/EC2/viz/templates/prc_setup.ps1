...
$PUBLISHED_ROOT = "${Fileshare_IP}\viz\published"
...
[Environment]::SetEnvironmentVariable("EGIS_USERNAME", "hydrovis.proc", "2")
...
Set-Location -Path $VIZ_DIR
& .\owp-viz-services-aws\aws_loosa\ec2\deploy\create_windows_services.ps1 ${WINDOWS_SERVICE_STARTUP} ${WINDOWS_SERVICE_STATUS} ${PIPELINE_USER} ${PIPELINE_USER}_ACCOUNT_PASSWORD
