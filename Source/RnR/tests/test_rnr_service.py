...
def test_mapped_feature_id(feature_id: int = 2930769, lid: str = "CAGM7") -> None:
    ...
    mapped_feature_id = rnr.map_ds_feature_id(feature_id, mock_cache(), gpkg_file)
    assert mapped_feature_id == "1074885", "mapping function giving the wrong divide id"


def test_assimilation_mapping(feature_id: int = 2930769, lid: str = "CAGM7") -> None:
    gpkg_file = Path(__file__).parent.absolute() / "test_data/2930769/subset.gpkg"
    mapped_feature_id = rnr.map_feature_id(feature_id, mock_cache(), gpkg_file)
